...
if (Sys.getenv("RSTUDIO") == "1") { setwd("/home/octavel/PhD_Programs/git_repos/are-we-fast-yet/report") }
...
vm_names <- c(
  ...
  "GraalJS-NativeEE-int" = "GraalJS EE 22.0.0.2 Native Int",
...
vms_slow <- c("JRubyC2", "JRubyGraal", "JRubyJ8",
              ...
              "Pharo", "SOMnsInt", "Topaz",  "TruffleSOM-ast-NativeEE-int-super",  "TruffleSOM-ast-NativeEE-int-main",
              "GraalPython-NativeEE-int", "TruffleRuby-NativeEE-int", "GraalJS-NativeEE-int")
vms_fast <- c("Crystal",
              "GraalBasic", "GraalC2", "GraalEnterprise",
              "GraalJS-HotspotEE-jit",
              "JRubyTruffle", "TruffleRuby-HotspotEE-jit",
              "Java8U66",
              "PyPy-jit",
              "GraalPython-HotspotEE-jit",
              "Node", "Node-jit",
              "SOMns", "SOMns-Enterprise",
              "TruffleSOM", "TruffleSOM-TOM", "TruffleSOM-ast-HotspotCE-jit-main", "TruffleSOM-TOM-Enterprise")
vms_truffle <- c("GraalJS-HotspotEE-jit",
                 "JRubyTruffle", "TruffleRuby-HotspotEE-jit",
                 "TruffleSOM", "TruffleSOM-ast-HotspotCE-jit-main", "TruffleSOM-TOM", "TruffleSOM-TOM-Enterprise",
                 "SOMns", "SOMns-Enterprise")
...
base <- data %>%
  filter(exe == "Node-int") %>% #name of the baseline
  group_by(bench,
...
#  stats <- stats_latest
  ...
  plot <- plot + geom_hline(aes(yintercept=1), colour="#cccccc", linetype="dashed")
  plot <- plot + geom_hline(aes(yintercept=2), colour="#cccccc", linetype="dashed")
  plot <- plot + geom_hline(aes(yintercept=3), colour="#cccccc", linetype="dashed")
  plot <- plot + geom_hline(aes(yintercept=5), colour="#cccccc", linetype="dashed")
  plot <- plot + geom_hline(aes(yintercept=10), colour="#cccccc", linetype="dashed")
  plot <- plot + geom_hline(aes(yintercept=20), colour="#cccccc", linetype="dashed")
  
  plot <- plot +
    geom_boxplot(outlier.size = 0.5) + #fill=get_color(5, 7)
    theme_bw() + theme_simple(font_size = 9) +
    ...
    #ggtitle("Runtime Factor, normalized to Node.js\n(lower is better)") + 
    coord_flip() + xlab("") +
...
p + scale_y_continuous(breaks = c(1, 2, 3, 4, 6, 8, 10, 12))

#p <- overview_box_plot(stats_latest, vms_all)
p <- overview_box_plot(stats_latest, c("Node-jit", "PyPy-jit", "GraalJS-HotspotEE-jit", "TruffleRuby-HotspotEE-jit", "GraalPython-HotspotEE-jit", "TruffleSOM-ast-HotspotCE-jit-main"))
p <- p + geom_hline(aes(yintercept=1), colour="#cccccc", linetype="dashed")
p <- p + scale_y_continuous(breaks = c(0, 5, seq(from=0, by=1, to=10)))
p

p <- overview_box_plot(stats_latest, c("Node-int", "CRuby-int", "CPython-int", "TruffleSOM-ast-NativeEE-int-super", 
                                       "TruffleSOM-ast-NativeEE-int-main", "GraalJS-NativeEE-int",
                                       "TruffleRuby-NativeEE-int", "GraalPython-NativeEE-int"), new_colors = TRUE)
p <- p + scale_y_continuous(breaks = c(1, 2, 3, 5, 10, 20))
p
