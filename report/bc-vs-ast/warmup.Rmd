...
data <- rbind(
  load_data_url("https://rebench.stefan-marr.de/Are-We-Fast-Yet/data/3048"), # all jit data
  load_data_url("https://rebench.stefan-marr.de/TruffleSOM/data/2867"), # older data with Native runs except Havlak and CD
  load_data_url("https://rebench.stefan-marr.de/TruffleSOM/data/3190") # Havlak and CD on Native
) %>% 
  subset(!(exe %in% c("TruffleSOM-native-interp-ast", "TruffleSOM-native-interp-bc", "RPySOM-bc-interp", "RPySOM-ast-interp", "Java20-C2-jit", "Node-jit"))) %>%
  subset(iteration <= 500)
...
data$exe <- as.character(data$exe)
data$exe[data$exe == "TruffleSOM-ast-HotspotCE-jit"] <- "TruffleSOM-graal-ast"
data$exe[data$exe == "TruffleSOM-bc-HotspotCE-jit"] <- "TruffleSOM-graal-bc"
data$exe[data$exe == "TruffleSOM-graal"] <- "TruffleSOM-graal-ast"
...
sorted_benchmarks <- c(
  ...
  "List", "Mandelbrot", "NBody", "Permute",
  "Queens", "Richards", "Sieve", "Storage", "Towers"
)
