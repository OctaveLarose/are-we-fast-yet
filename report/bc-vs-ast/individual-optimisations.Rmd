...
compute_stats <- function(data, exe_name, iteration_threshold = 0, baseline_optim="all optimizations") {
  # data <- data_optim_tsom_ast; exe_name <- "TruffleSOM-native-interp-ast"
  ...
  per_bench <- norm |>
    ...
    summarize(
      ...
      .groups = "drop")
  per_bench$optim <- factor(per_bench$optim)
  per_bench$exe <- factor(per_bench$exe)
...
get_and_format_new_data <- function(data) {
  ...
  data$exe[data$exe == "TruffleSOM-ast-NativeCE-int-astvsbc"] <- "TruffleSOM-native-interp-ast"
  data$exe[data$exe == "TruffleSOM-bc-NativeCE-int"] <- "TruffleSOM-native-interp-bc"
  data$exe[data$exe == "TruffleSOM-bc-NativeCE-int-astvsbc"] <- "TruffleSOM-native-interp-bc"
...
tsom_bc_interps <- load_data_url("https://rebench.stefan-marr.de/Are-We-Fast-Yet/data/3083") |>
  # fix up data mislabeling
  mutate(
    exe = ifelse(exe == "TruffleSOM-bc-NativeCE-int-no-lower-prims" & str_detect(cmdline, "TruffleSOM/som-native-interp-bc"),
                 "TruffleSOM-bc-NativeCE-int",
                 as.character(exe)))
tsom_bc_interps$exe <- factor(tsom_bc_interps$exe)
...
assert_that_all_optimizations_have_all_benchmarks <- function(box_plot_data) {
  for (o in levels(box_plot_data$optim)) {
    a <- box_plot_data |>
      filter(optim == o) |>
      droplevels()
    assert_that(length(levels(a$bench)) == 14)
  }
}
```
...
assert_that_all_optimizations_have_all_benchmarks(box_plot_data)
